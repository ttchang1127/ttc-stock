...
> 📌 假設來源：`dcf_assumptions.json`（沿用先前 thesis 標題：常態化 FCF g=13%, WACC=8.5%）。基期自由現金流 $60,391 百萬，預測 10 年後接終端價值，共 10,000 次有效模擬。

| 分位 | P5 | P25 | P50 (中位數) | P75 | P95 |
|---|---|---|---|---|---|
| 每股內在價值 | $609.17 | $769.90 | **$911.97** | $1,084.54 | $1,415.45 |

- **平均內在價值**: **$947.79**
- **50% 主流估值區間 (P25 ~ P75)**: **$769.90 ~ $1,084.54**
- **現價 $592.10 相對中位數**: 低於中位數 **35.1%**
- **現價隱含的 FCF 年成長率**: **7.4%**（在 WACC 8.50% 與同組終端假設下，讓模型值等於現價所需的成長率）

> ⚠️ 當期 FCF 46,109M 為常態化基準 60,391M 的 76%（歷年 FCF 利潤率中位數 30.1%）。基期採常態化值，以免單一年度的資本支出高峰被外推十年

> 🚨 **中位數 $911.97 與現價 $592.10 相差 +54%，超過 ±50% 門檻。這個幅度通常反映假設不適用，而非市場錯價 —— 請以下方的隱含成長率判讀，不要把中位數當成目標價**

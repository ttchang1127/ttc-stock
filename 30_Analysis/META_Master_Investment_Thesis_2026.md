...
- **近 3 年 Sortino Ratio（週資料）**: **1.28**　（2023-08-25 ~ 2026-08-24，157 週報酬）
- **近 5 年 Sortino Ratio（週資料）**: **0.62**　（2021-08-27 ~ 2026-08-24，261 週報酬）

**對照公開篩選器（日資料）**

- **近 12 個月 Sortino Ratio（日資料，MAR=0）**: **-0.79**　（2025-08-25 ~ 2026-08-24，250 個交易日）
  - 若改以無風險利率為門檻（3.68%，^IRX 13週美國國庫券，期間平均）則為 **-0.91**
...
> 📌 本節全部由 SEC 財報數字與 `prices.json` 收盤價計算，股價為 2026-08-24 之 $559.02。
...
- **現價 $559.02 相對中位數**: 低於中位數 **41.9%**
- **現價隱含的 FCF 年成長率**: **11.0%**（在 WACC 10.57% 與同組終端假設下，讓模型值等於現價所需的成長率）

> ⚠️ 當期 FCF 46,109M 為常態化基準 60,391M 的 76%（歷年 FCF 利潤率中位數 30.1%）。基期採常態化值，以免單一年度的資本支出高峰被外推十年

> 🚨 **中位數 $961.69 與現價 $559.02 相差 +72%，超過 ±50% 門檻。這個幅度通常反映假設不適用，而非市場錯價 —— 請以下方的隱含成長率判讀，不要把中位數當成目標價**

...
> 📌 假設來源：`dcf_assumptions.json`（沿用先前 thesis 標題：常態化 FCF g=12%, WACC=8.5%）。基期自由現金流 $22,321 百萬，預測 10 年後接終端價值，共 10,000 次有效模擬。

| 分位 | P5 | P25 | P50 (中位數) | P75 | P95 |
|---|---|---|---|---|---|
| 每股內在價值 | $53.85 | $66.47 | **$77.67** | $91.25 | $116.69 |

- **平均內在價值**: **$80.48**
- **50% 主流估值區間 (P25 ~ P75)**: **$66.47 ~ $91.25**
- **現價 $274.48 相對中位數**: 高於中位數 **253.4%**
- **現價隱含的 FCF 年成長率**: **29.5%**（在 WACC 8.50% 與同組終端假設下，讓模型值等於現價所需的成長率）

> ⚠️ 當期 FCF 7,695M 為常態化基準 22,321M 的 34%（歷年 FCF 利潤率中位數 3.1%）。基期採常態化值，以免單一年度的資本支出高峰被外推十年

> 🚨 **中位數 $77.67 與現價 $274.48 相差 -72%，超過 ±50% 門檻。這個幅度通常反映假設不適用，而非市場錯價 —— 請以下方的隱含成長率判讀，不要把中位數當成目標價**

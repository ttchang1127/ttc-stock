...
- **近 3 年 Sortino Ratio（週資料）**: **0.82**　（2023-08-25 ~ 2026-08-19，156 週報酬）
- **近 5 年 Sortino Ratio（週資料）**: **0.68**　（2021-08-20 ~ 2026-08-19，261 週報酬）

**對照公開篩選器（日資料）**

- **近 12 個月 Sortino Ratio（日資料，MAR=0）**: **0.53**　（2025-08-19 ~ 2026-08-19，251 個交易日）
  - 若改以無風險利率為門檻（3.72%，^IRX 13週美國國庫券，期間平均）則為 **0.41**
...
> 📌 本節全部由 SEC 財報數字與 `prices.json` 收盤價計算，股價為 2026-08-19 之 $351.12。
...
- **現價 $351.12 相對中位數**: 高於中位數 **299.1%**
- **現價隱含的 FCF 年成長率**: **46.9%**（在 WACC 13.19% 與同組終端假設下，讓模型值等於現價所需的成長率）

> 🚨 **中位數 $87.97 與現價 $351.12 相差 -75%，超過 ±50% 門檻。這個幅度通常反映假設不適用，而非市場錯價 —— 請以下方的隱含成長率判讀，不要把中位數當成目標價**

...
- **近 3 年 Sortino Ratio（週資料）**: **0.74**　（2023-08-11 ~ 2026-08-11，157 週報酬）
- **近 5 年 Sortino Ratio（週資料）**: **0.63**　（2021-08-13 ~ 2026-08-11，261 週報酬）

**對照公開篩選器（日資料）**

- **近 12 個月 Sortino Ratio（日資料，MAR=0）**: **0.27**　（2025-08-11 ~ 2026-08-11，251 個交易日）
  - 若改以無風險利率為門檻（3.70%，^IRX 13週美國國庫券，期間平均）則為 **0.16**
...
> 📌 本節全部由 SEC 財報數字與 `prices.json` 收盤價計算，股價為 2026-08-11 之 $332.81。
...
- **現價 $332.81 相對中位數**: 高於中位數 **278.3%**
- **現價隱含的 FCF 年成長率**: **46.1%**（在 WACC 13.19% 與同組終端假設下，讓模型值等於現價所需的成長率）

> 🚨 **中位數 $87.97 與現價 $332.81 相差 -74%，超過 ±50% 門檻。這個幅度通常反映假設不適用，而非市場錯價 —— 請以下方的隱含成長率判讀，不要把中位數當成目標價**

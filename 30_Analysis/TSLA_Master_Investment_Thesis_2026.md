...
- **近 3 年 Sortino Ratio（週資料）**: **0.90**　（2023-08-18 ~ 2026-08-14，156 週報酬）
- **近 5 年 Sortino Ratio（週資料）**: **0.67**　（2021-08-20 ~ 2026-08-14，260 週報酬）

**對照公開篩選器（日資料）**

- **近 12 個月 Sortino Ratio（日資料，MAR=0）**: **0.39**　（2025-08-14 ~ 2026-08-14，251 個交易日）
  - 若改以無風險利率為門檻（3.69%，^IRX 13週美國國庫券，期間平均）則為 **0.28**
...
> 📌 本節全部由 SEC 財報數字與 `prices.json` 收盤價計算，股價為 2026-08-14 之 $342.27。
...
- **現價 $342.27 相對中位數**: 高於中位數 **289.1%**
- **現價隱含的 FCF 年成長率**: **46.5%**（在 WACC 13.19% 與同組終端假設下，讓模型值等於現價所需的成長率）

> 🚨 **中位數 $87.97 與現價 $342.27 相差 -74%，超過 ±50% 門檻。這個幅度通常反映假設不適用，而非市場錯價 —— 請以下方的隱含成長率判讀，不要把中位數當成目標價**

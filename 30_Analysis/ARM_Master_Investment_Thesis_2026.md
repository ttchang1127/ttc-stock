...
- **近 3 年 Sortino Ratio（週資料）**: **1.94**　（2023-09-22 ~ 2026-09-17，156 週報酬）
- **近 5 年 Sortino Ratio（週資料）**: 資料不足（股價歷史未滿 5 年，僅 157 週報酬，起始 2023-09-15）

**對照公開篩選器（日資料）**

- **近 12 個月 Sortino Ratio（日資料，MAR=0）**: **1.80**　（2025-09-17 ~ 2026-09-17，251 個交易日）
...
> 📌 本節全部由 SEC 財報數字與 `prices.json` 收盤價計算，股價為 2026-09-17 之 $263.71。
...
- **現價 $263.71 相對中位數**: 高於中位數 **1443.1%**
- **現價隱含的 FCF 年成長率**: **64.6%**（在 WACC 17.91% 與同組終端假設下，讓模型值等於現價所需的成長率）

> 🚨 **中位數 $17.09 與現價 $263.71 相差 -94%，超過 ±50% 門檻。這個幅度通常反映假設不適用，而非市場錯價 —— 請以下方的隱含成長率判讀，不要把中位數當成目標價**

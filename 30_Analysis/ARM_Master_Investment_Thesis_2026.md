...
- **近 3 年 Sortino Ratio（週資料）**: 資料不足（股價歷史未滿 3 年，僅 156 週報酬，起始 2023-09-15）
- **近 5 年 Sortino Ratio（週資料）**: 資料不足（股價歷史未滿 5 年，僅 156 週報酬，起始 2023-09-15）

**對照公開篩選器（日資料）**

- **近 12 個月 Sortino Ratio（日資料，MAR=0）**: **2.01**　（2025-09-08 ~ 2026-09-08，251 個交易日）
  - 若改以無風險利率為門檻（3.67%，^IRX 13週美國國庫券，期間平均）則為 **1.92**
...
> 📌 本節全部由 SEC 財報數字與 `prices.json` 收盤價計算，股價為 2026-09-08 之 $258.34。
...
- **現價 $258.34 相對中位數**: 高於中位數 **1411.6%**
- **現價隱含的 FCF 年成長率**: **64.3%**（在 WACC 17.91% 與同組終端假設下，讓模型值等於現價所需的成長率）

> 🚨 **中位數 $17.09 與現價 $258.34 相差 -93%，超過 ±50% 門檻。這個幅度通常反映假設不適用，而非市場錯價 —— 請以下方的隱含成長率判讀，不要把中位數當成目標價**
